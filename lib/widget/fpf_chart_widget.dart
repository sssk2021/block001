...
// FP Fグラフ表示
class FpFChartWidget extends ConsumerStatefulWidget {
  FpFChartWidget({
    Key? key,
  ...
  @override
  _FpFChartWidget createState() => _FpFChartWidget();
}

class _FpFChartWidget extends ConsumerState<FpFChartWidget> {
  ...
  List<LineSeries<_ChartData, DateTime>> _getDefaultLineSeries() {
    ...
    for (int i = 0; i < _sampList.length; i++) {
      ...
      _ChartData cd = _ChartData(
        x_dt,
        _sampList[i].Fx.toDouble(),
        _sampList[i].Fy.toDouble(),
        _sampList[i].Fz.toDouble(),
        color,
      );
